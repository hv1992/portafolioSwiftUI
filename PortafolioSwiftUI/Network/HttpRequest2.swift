...
actor HttpRequest2 : GlobalActor{
    ...
    func requestCodable<T : Codable>(urlString : String,queryUrl : [String:String]?,parametersBody : JSON?,parametersHeaders : [String : String]?,httpMethod : TypeHttpEnum = TypeHttpEnum.get) async throws -> T {
        return try await withCheckedThrowingContinuation { continuation in
            
            //Esto si por si agregamos query a la llamada
            var components = URLComponents(string: urlString)
            components?.queryItems = []
            queryUrl?.forEach { components?.queryItems?.append(URLQueryItem(name: $0.key, value: $0.value)) }
            
            //Esto es por si al final no hubo ningun query, y por lo tanto, se restaura como estaba antes (no se pone una array vacio)
    ...
    ///   - parametersHeaders: parametros que se van a ir en el header
    ///   - httpMethod: el tipo de llamada http que se va hacer (GET, POST, ...)
    /// - Returns: retorna un array de objeto que hereda de Codable.
    func requestArrayCodable<T : Codable>(urlString : String,queryUrl : [String:String]?, parametersBody : JSON? = nil,parametersHeaders : [String : String]? = nil,httpMethod : TypeHttpEnum = TypeHttpEnum.get) async throws -> [T] {
        return try await withCheckedThrowingContinuation { continuation in
            
            
            //Esto si por si agregamos query a la llamada, que van a estar concatenado con la url
            var components = URLComponents(string: urlString)
            components?.queryItems = []
            queryUrl?.forEach { components?.queryItems?.append(URLQueryItem(name: $0.key, value: $0.value)) }
